<template>
  ...
  box-shadow: 0 8px 4px -4px rgba(0,0,0,.5);</code></pre>
            <div class="exp shadow-2">
                <div></div>
            </div>
            <p>叠加上面的效果，可以得到<strong>双侧投影：</strong></p>
            <div class="exp shadow-3">
                <div></div>
            </div>
            <pre><code>background: #58a;
  box-shadow: 8px 0 4px -4px rgba(0,0,0,.5), -8px 0 4px -4px rgba(0,0,0,.5);</code></pre>
            ...
            <div class="exp cuttingAngle-3 filter-1">
                <div></div>
            </div>
  ...
  &:hover{
    filter: none
            ...
            <p><strong>基于混合模式的方案：</strong>在Adobe
                Photo中混合模式定义了上层元素的颜色与下层元素的颜色进行混合的方式。使用luminosity模式可以实现染色效果。这种模式会保留上层元素的HSL亮度信息，并从它的下层吸取色相饱和度信息。</p>
  ...
  &:before{
    ...
    background: linear-gradient(to left bottom, transparent 50%, rgba(0,0,0,0.4) 0);
            ...
            <div class="exp angle-3">
                <div></div>
            </div>
            <p>要做到真实的效果，需要将折页三角形的宽和高对调，向上偏移一定的位置并旋转30度。</p>
            <div class="exp angle-3 angle-4">
                <div></div>
            </div>
            <pre><code>&:before{
  width:1.73em;
  ...
  transform-origin: bottom right;
            ...
            <div class="exp angle-3 angle-4 angle-5">
                <div></div>
            </div>
...
<script>
    ...
    module.exports = {
        mounted: function () {
            ...
            Global.setArticleNavMenu('learning-css-secrects-4')
        }
    }
</script>
