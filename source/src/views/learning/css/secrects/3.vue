<template>
    <div class="page article">
        ...
        <div class="content">
            ...
            <p>border-radius支持四个角设置成不同的水平和垂直半径。方法是在斜杠之前指定1~4个值，在斜杠之后指定另外的1~4个值。如10px / 5px 20px，展开为10px 10px 10px 10px /
                5px 20px 5px 20px。四个角的水平半径为10px，垂直半径则分别为5,20,5,20px。</p>
            <p>半椭圆效果：<strong>border-radius:50% / 100% 100% 0 0</strong></p>
            <div class="exp ellipse-2">
                <div></div>
            </div>
            <p>沿着纵轴劈开的半椭圆效果：<strong>border-radius:100% 0 0 100% / 50%</strong></p>
            <div class="exp ellipse-3">
                <div></div>
            </div>
            <p>四分之一椭圆效果：<strong>border-radius: 100% 0 0 / 100% 0 0</strong></p>
            <div class="exp ellipse-4">
                <div></div>
            </div>
            <h3 class="title">平行四边形</h3>
            <p>在视觉设计中，平行四边形可以表达出一种动感。</p>
            <p><strong>方法一：嵌套元素抵消变形</strong></p>
            <div class="exp paralle-1">
                <div>
                    <div>平行四边形</div>
                </div>
            </div>
...
&:before{
  ...
  animation: pie-spin 3s linear infinite, pie-bg 6s step-end infinite;
            ...
            <p><strong>负的动画时延（animation-delay）会直接跳至动画中的任意时间点，并定格在那里。</strong>如果动画持续时间为6秒，animation-delay设置为-1.2s可以使饼图显示出20%的比率。为计算简单考虑，可以设置动画持续时间为100s。
            </p>
            <p>因为动画是设置在伪元素上的，为方便灵活显示不同的比率，可在主元素内联设置animation-delay，伪元素则<strong>inherit</strong>这个值。</p>
            <div class="exp pie pie-4">
...
<script>
    ...
    module.exports = {
        mounted: function () {
            ...
            Global.setArticleNavMenu('learning-css-secrects-3')
        }
    }
</script>
